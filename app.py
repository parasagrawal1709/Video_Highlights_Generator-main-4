...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def generate_summary(self):
        self.setMovie()
        self.gif.start()
        if self.summarization == 'coach':
            print("RUNNING FOR COACH")
        ...
        elif self.summarization == 'audience':
            # movie 
            print("RUNNING FOR AUDIENCE")
            self.thread = ThreadClass(isMovie = self.summarization, filePath = self.filePath, fileName= self.fileName)
            self.thread.start()
            self.thread.update.connect(self.output)
        else:
...
class ThreadClass(QtCore.QThread):
    ...
    def run(self):
        # first download youtube video then generate coach highlights
        if self.isMovie == 'coach':
            try:
